...
export default function JobDetailMock() {
  ...
  // --- Dev-time smoke tests (non-breaking) ---
  React.useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.assert(!!job.title, "[test] job.title should be defined");
      console.assert(Array.isArray(job.tags) && job.tags.length > 0, "[test] job.tags should have items");
      console.assert(typeof job.salary === "string", "[test] job.salary should be a string");
    }
  }, []);
          ...
          <Card className="shadow-sm">
            <CardHeader className="pb-4">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <h1 data-testid="job-title" className="text-2xl font-semibold tracking-tight">{job.title}</h1>
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <span className="inline-flex items-center gap-1"><Clock className="h-4 w-4" />Posted {job.posted}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
