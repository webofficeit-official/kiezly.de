...
// ------------------------------
function TagInput({ name, label, value, onChange, suggestions = [], placeholder = "Type a skill and press Enter", required = false, error }) {
  ...
  const lowerSelected = React.useMemo(() => new Set((value || []).map((v) => v.toLowerCase())), [value]);
  const filtered = React.useMemo(() => {
    const q = input.trim().toLowerCase();
    if (!q) return suggestions.filter((s) => !lowerSelected.has(String(s).toLowerCase())).slice(0, 8);
    return suggestions
      .filter((s) => String(s).toLowerCase().includes(q) && !lowerSelected.has(String(s).toLowerCase()))
      .slice(0, 8);
  ...
  function addTag(tag) {
    ...
    if (!t) return;
    if (lowerSelected.has(t.toLowerCase())) return;
    onChange([...(value || []), t]);
    setInput("");
    setActiveIndex(-1);
    setOpen(false);
  ...
  return (
    <div>
      ...
      {/* Hidden input to participate in FormData */}
      <input type="hidden" name={name} value={(value || []).join(", ")} />

      <div className={`w-full rounded-xl border px-2 py-1 ${error ? "border-red-400 ring-2 ring-red-100" : "border-gray-300"}`}
           onClick={() => inputRef.current?.focus()}>
        <div className="flex flex-wrap gap-2">
          {(value || []).map((tag) => (
            <span key={tag} className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-1 text-xs">
              {tag}
              <button type="button" className="ml-1 rounded p-0.5 hover:bg-gray-200" aria-label={`Remove ${tag}`} onClick={() => removeTag(tag)}>
                ×
              </button>
            </span>
          ))}
