...
export default function UpdateStatusModal({ isOpen, onClose, applicant }) {
  ...
  const handleUpdateStatus = () => {
    ...
    updateStatus.mutate(
      ...
      },
      {
        ...
        onError: (err: any) => {
          toast.error(
            err?.message || t("applicants.update-model.update.failed")
          );
          setIsButtonDisabled(false);
        },
      }
  ...
  const statusOptions = [
    {
      label: t("applicants.update-model.status.options.shortlisted"),
      value: "shortlisted",
    },
    {
      label: t("applicants.update-model.status.options.accepted"),
      value: "accepted",
    },
    {
      label: t("applicants.update-model.status.options.rejected"),
      value: "rejected",
    },
  ];
        ...
        <div className="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <div className="bg-white px-6 py-4 border-b border-gray-100">
            <h3
              ...
              id="modal-title"
            >
              {t("applicants.update-model.title", {
                name: `${applicant.user.first_name} ${applicant.user.last_name}`,
              })}
            </h3>
            <p className="text-sm text-gray-500 mt-1">
              {t("applicants.update-model.current-status")}{" "}
            ...
            <div className="text-sm text-gray-700 mt-4">
              <p>
                <span className="font-semibold text-gray-800">
                  {t("applicants.update-model.rate")}
                </span>{" "}
          ...
          <div className="bg-gray-50 px-6 py-4 flex justify-end gap-3">
            ...
            <Button
              ...
              disabled={isButtonDisabled || updateStatus.isPending}
            >
              {updateStatus.isPending || isButtonDisabled? (
                <>
                  <span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span>
                  {t("applicants.update-model.button.updating")}
                </>
              ) : (
