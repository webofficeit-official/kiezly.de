...
export default function MyJobs({
  ...
  const apiFilters = useMemo(
    () => buildApiFilters(debouncedFilters, page, pageSize),
  ...
  const CloseJobButton = React.forwardRef<
    HTMLButtonElement,
  ...
  const statusOptionsTranslation = (
    [
      "draft",
      "pending_review",
      "open",
  ...
  ).map((v) => ({ label: t(`filters.status.${v}`), value: v }));
        ...
        <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border p-4 sm:p-6 flex items-center justify-between gap-10">
            ...
            <h2 className="text-lg font-semibold">
              {t("title_with_status")
                .replace("{{status}}", t(`filters.status.${filters.status}`))
                .replace("{{count}}", String(data?.data?.total_items ?? 0))}
            </h2>
            {/* Filters */}
            <div className="flex items-center justify-between gap-6">
              ...
              <Select
                ...
                options={statusOptionsTranslation}
                variant="status"
                t={(k) => t(k)} // pass through your translator
                placeholder={t("buttons.select")}
              />
            </div>
          </div>
...
function Select({
  ...
  const hasPending = options.some((o) => o.value === "pending_review");
  const localizedPending = t
    ? t("filters.status.pending_review")
    : "Pending review";

  const finalOptions =
    variant === "status" && !hasPending
      ? [{ label: localizedPending, value: "pending_review" }, ...options]
      : options;
  ...
  return (
    <div className="text-sm">
      <span className="mb-1 block text-gray-700">{labelText}</span>
