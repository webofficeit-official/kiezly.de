...
export default function ApplyPanel({ user, jobDetails }: ApplyPanelProps) {
  ...
  const { data: application, isLoading: isChecking } = useCheckApplied(
    jobDetails.id,
    {
      enabled: !!user,
    }
      ...
      {application?.success ? (
        application?.data?.application?.status == "withdrawn" ? (
          <ApplicationCard
            ...
            description={t("apply-panel.withdrawed.description", {
          ...
          <ApplicationCard
            ...
            description={t("apply-panel.applied.description", {
              title: jobDetails.title,
            })}
            coverNote={coverNote}
            proposedRate={proposedRate}
        ...
        <ApplicationCard
          ...
          buttonLabel={
            user
              ? t("apply-panel.apply.button.logged", {
                  name: `${user.first_name} ${user.last_name}`,
                })
              : t("apply-panel.apply.button.unauthenticate")
          }
      ...
      <div className="mt-6 space-y-2 text-sm text-muted-foreground">
        {jobDetails?.police_verified && (
          <div className="flex items-center gap-2">
            ...
            {t("apply-panel.mini-facts.police-verified")}
          </div>
        )}
        {jobDetails?.first_aid_verified && (
          <div className="flex items-center gap-2">
            <GraduationCap className="h-4 w-4" />{" "}
            {t("apply-panel.mini-facts.first-aid")}
          </div>
        )}
      </div>
    </aside>
