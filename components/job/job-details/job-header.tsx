      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1">
              ...
              {t("detail.header.posted")}{" "}
              {dayjs(jobDetails?.created_at).fromNow()}
            </span>
            ...
            {jobDetails?.starts_at && (
              <span className="inline-flex items-center gap-1">
                <Clock className="h-3 w-3" /> {t("detail.header.start")}:{" "}
                {dayjs(jobDetails.starts_at).format("MMM D, YYYY")}
              </span>
            )}

            {jobDetails?.ends_at && (
              <span className="inline-flex items-center gap-1">
                <Clock className="h-3 w-3" /> {t("detail.header.end")}:{" "}
                {dayjs(jobDetails.ends_at).format("MMM D, YYYY")}
              </span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" className="rounded-xl">
            <Share2 className="mr-2 h-4 w-4" /> {t("detail.header.share")}
          </Button>
          {savedJobs.some((j) => j.id === jobDetails.id) ? (
            ...
            >
              <Bookmark className="mr-2 h-4 w-4" /> {t("detail.header.save")}
            </Button>
          )}
