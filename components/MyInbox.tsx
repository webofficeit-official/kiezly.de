...
export default function MyInbox() {
    ...
    const { data: jobs, isLoading: jobLoading, error, isFetching } = myJobs({});
    const dataSource = jobs?.data?.items ?? [];
    ...
    const [selectedApplicantion, setSelectedApplicantion] = useState<Application | null>(null);
    ...
    const { data, isLoading: messageLoading } = useGetConversation(
        ...
        filters,
        { enabled: !!selectedJobId && !!selectedApplicantion?.user?.id }
    );
    ...
    useEffect(() => {
        ...
        setHasMore(true);
        isLoadingOlderRef.current = false;
    }, [selectedJobId, selectedApplicantion?.id]);
    ...
    const sendMessage = (e) => {
        ...
        sendMsg.mutate({
            jobId: selectedJobId,
            recipient_id: selectedApplicantion?.user.id,
            body: message
        }, {
            onSuccess: () => {
                setMessage("")
        ...
        <div className="mx-auto max-w-6xl px-4 py-8">
            ...
            <div className="flex h-[700px] w-full overflow-hidden bg-white shadow-sm">
                ...
                <div className="w-1/4 flex-shrink-0 border-r border-gray-100 bg-white">
                    ...
                    <div className="overflow-y-auto h-full">
                        {dataSource.map((d) => (
                            <button
                                key={d.id}
                                onClick={() => { setSelectedJobId(d.id); setSelectedApplicantion(null); }}
                                className={`w-full p-4 text-left border-b transition-colors ${selectedJobId === d.id ? 'bg-gray-50 border-r-4 border-r-gray-500' : 'hover:bg-gray-100'}`}
                            >
                                <p className="font-semibold text-gray-900">{d.title}</p>
                                <p className="text-xs text-gray-500 mt-1">{[d.city, d.state, d.countries?.name]
                                    .filter(Boolean)
                                    .join(", ")}</p>
                            </button>
                        ))}
                            ...
                            <div className="overflow-y-auto h-full">
                                {
                                    jobApplicants.length === 0 ? (
                                        ...
                                        jobApplicants?.map((a: Application, i) => (
                                            <button
                                                key={a.id}
                                                onClick={() => setSelectedApplicantion(a)}
                                                className={`w-full p-4 text-left border-b transition-colors ${selectedApplicantion?.id === a.id ? 'bg-gray-100 border-r-4 border-r-gray-500' : 'hover:bg-gray-50'}`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-700 font-bold">{a.user.first_name.charAt(0)}{a.user.last_name.charAt(0)}</div>
                                                    <div>
                                                        <p className="font-semibold text-gray-900">{a.user.first_name} {a.user.last_name}</p>
                                                        <p className="text-xs text-gray-500 mt-1">{a.user.email}</p>
                                                    </div>
                                                </div>
                                            </button>
                                        ))
                ...
                <div className="flex-grow flex flex-col bg-white">
                    {(userType === "helper" && selectedJobId) || (userType === "client" && selectedApplicantion) ? (
                        <>
                            {/* Chat Header */}
                            <div className="p-4 border-b flex items-center justify-between">
                                <div>
                                    <h3 className="font-semibold">{userType === "client" ? `${selectedApplicantion.user.first_name} ${selectedApplicantion.user.last_name} / ${selectedApplicantion.proposed_rate}` : "Client"}</h3>
                                    <div className="font-medium text-xs text-gray-600" dangerouslySetInnerHTML={{ __html: selectedApplicantion.cover_note }} />
                                </div>
                                <span className="h-2 w-2 rounded-full bg-green-500"></span>
                            </div>

                            {/* Chat Messages */}
                            <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-grow p-6 overflow-y-auto space-y-4 bg-gray-50/30">
                                {
                                    messages.map((msg, i) => (
                                        <div
                                            key={i}
                                            className={`flex ${msg.recipient_id === selectedApplicantion.user.id
                                                ? "justify-end"
                                                : "justify-start"
                                                }`}
                                        >
                                            <div className="max-w-[75%]">
                                                <div
                                                    className={`rounded-2xl px-4 py-2 text-sm shadow-sm ${msg.recipient_id === selectedApplicantion.user.id
                                                ...
                                                <p
                                                    className={`mt-1 text-[10px] text-muted-foreground ${msg.recipient_id === selectedApplicantion.user.id
                                                        ? "text-right"
                                                        : "text-left"
                                                        }`}
                                                >
                        ...
                        <div className="flex h-full flex-col items-center justify-center text-gray-400">
                            ...
                            <p>{userType === "client" ? "Select an applicant to start chatting" : "Select a job to view your messages"}</p>
                        </div>
                    )}
                </div>
            </div>
